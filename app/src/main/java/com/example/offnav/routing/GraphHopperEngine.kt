...
class GraphHopperEngine(private val context: Context) {
    ...
    @Volatile
    var state: RoutingState = RoutingState.NotReady
        private set
    ...
    suspend fun initialize() = withContext(Dispatchers.IO) {
        initMutex.withLock {
            ...
            try {
                ...
                if (needsImport) state = RoutingState.ImportingGraph
                ...
                gh.importOrLoad() // loads existing graph, or imports pbf
                hopper = gh
                state = RoutingState.Ready
            } catch (e: Exception) {
                state = RoutingState.Failed(e.message ?: "Graph init failed")
            }
        }
    }
